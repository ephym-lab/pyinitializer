...
def _scaffold_django(tree: FileTree, config: ProjectConfig) -> None:
    n = config.project_name
    mn = config.module_name

    # manage.py
    tree["manage.py"] = dedent(f"""\
        #!/usr/bin/env python
        import os
        import sys
    ...
    tree[f"{mn}/settings.py"] = dedent(f"""\
        ...
        # Add requested apps
        {chr(10).join(f"INSTALLED_APPS.append('{app}')" for app in config.django_apps)}
